...
namespace TileSurvival
{
	public class Perlin
	{
		...
		public static float Perlin2D(Vector2 position)
        {
			return (float)perlin(position.X, position.Y, 1);
		}
		public static float Perlin2D(Vector2 position, int octaves, float persistance = 0.25f)
		{
			return (float)OctavePerlin(position.X, position.Y, 1, octaves, persistance);
		}
